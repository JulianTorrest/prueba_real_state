...
@st.cache_data
def load_limited_data(url, num_rows=200_000):
    try:
...
df = load_limited_data(PARQUET_URL, num_rows=200_000)
...
def load_and_preprocess_data(url): # Eliminado nrows de los argumentos
    ...
    ya que el formato contiene su propio esquema y es más eficiente.
    """
    ...
    try:
        ...
        st.error(f"Error al intentar cargar o preprocesar el archivo Parquet: {e}")
        st.info("Asegúrate de que la URL del archivo Parquet sea correcta y que el archivo sea válido.")
        st.exception(e)
        return pd.DataFrame()
